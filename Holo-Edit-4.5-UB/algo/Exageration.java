 ...
 */
public class Exageration extends Algorithm
{
	public Exageration(GestionPistes gp)
	{
		super(gp,TYPE_TRANS_ATOB,"Exageration","Exageration","Exagerate the local movement of the trajectory\nThe locality is tuned by the <locality> parameter in seconds");
		...
		addField(new Field("Locality (sec)", Param.TYPE_DOUBLE, 1., 0.05, 10.));
		addField(new Field("Proportion X (%)", Param.TYPE_DOUBLE, 200., -1000., 1000.));
		addField(new Field("Proportion Y (%)", Param.TYPE_DOUBLE, 200., -1000., 1000.));
		addField(new Field("Proportion Z (%)", Param.TYPE_DOUBLE, 200., -1000., 1000.));
	...
	protected void treatOneTrack(int tkNthFrom, int tkNthTo)
	{
		...
		HoloPoint ctrlPoint ;
		HoloPoint outPoint = new HoloPoint();
		
		// --- parametres fournis par l'utilisateur :
		double smoothness = (Double)results[3];
		double winLength = smoothness * 1000;
		double pts_ps = 20.; // cuurently unused
		float ampX = (float)((Double)results[4]/100);
		float ampY = (float)((Double)results[5]/100);
		float ampZ = (float)((Double)results[6]/100);
...
//			 pad fin
			...
			for(HoloPoint inPoint : ht.points)
			{
				// remise a zeros des param de boucle
				curPoint.x = 0;
				curPoint.y = 0;
				curPoint.z = 0;
				curTime = inPoint.date;
				coefSum = 0;

				//si 1er ou dernier element ->  copie
				if(inPoint == ht.points.firstElement())
					finalPts.addElement(ctrlPts.get(padBegin).dupliquer());
				else if(inPoint == ht.points.lastElement())
				...
				else
				{
					...
					outPoint.date = curTime;
					outPoint.x = (inPoint.x - curPoint.x) * ampX + curPoint.x;
					outPoint.y = (inPoint.y - curPoint.y) * ampY + curPoint.y; 
					outPoint.z = (inPoint.z - curPoint.z) * ampZ + curPoint.z; 
					finalPts.addElement(outPoint.dupliquer());
				}
